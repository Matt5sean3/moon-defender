...
// === START MENU SCREEN
function MenuScreen(ctx, renderFunction) {
...
function MenuOption(loc, renderFunction, bbox, event) {
	this.loc = loc;
	this.render = renderFunction;
	this.bbox = bbox;
	this.event = event;
...
// === END MENU SCREEN
function renderMenu(ctx, currentTime, dt) {
    // placeholder
    ctx.font = "20px Arial";
...
function renderPlayButton(ctx, currentTime, dt) {
	// placeholder
	ctx.font = "20px Arial";
